...
let video = document.querySelector('.js-bgvideo');
if(document.querySelector('.js-bgvideo')){
	let source = video.querySelectorAll('source');
	source.forEach( item =>{
		let src = item.getAttribute('data-url');
		if(isMobile.any()){
...
jQuery( function( $ ) {
	...
	$( 'body' ).on( 'click', '.cart-table .remove', function() {
		timeout = setTimeout(function() {
			$('#header-cart-content').load(location.href+' #header-cart-content>a');
		}, 500 );
	} );
	$( document ).on( "ajaxComplete", function() {
		...
		if($('.woocommerce-cart').length) {
			...
			if ($('#header-cart-content a span').text()) {
