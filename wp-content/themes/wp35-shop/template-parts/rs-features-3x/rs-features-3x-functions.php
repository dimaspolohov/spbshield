<?php

// Enqueue 3-column features block styles
add_action( 'wp_enqueue_scripts', 'style_rs_features_3x_theme' );

function style_rs_features_3x_theme() {
	wp_enqueue_style( 'rs-features-3x', get_stylesheet_directory_uri() . '/template-parts/rs-features-3x/css/rs-features-3x.css' );
}

// Render 3-column features block (block_id=6)
function storefront_rs_features_3x() {
	$post_meta   = null;
	$title       = '';
	$description = '';

	$query = new WP_Query( array(
		'post_type'  => 'custom_block',
		'meta_query' => array(
			'relation' => 'OR',
			array(
				'key'     => 'block_id',
				'value'   => 6,
				'compare' => '=',
			),
		),
	) );

	while ( $query->have_posts() ) {
		$query->the_post();
		$post_meta = get_post_meta( $query->post->ID );
	}

	if ( $post_meta ) {
		$title       = get_field( 'preference_3x_title' ) ?: '';
		$description = get_field( 'preference_3x_text' ) ?: '';
	}
	?>
	<section class="rs-17">
		<div class="rs-features index-page">
			<div class="container">
				<?php if ( $title || $description ) : ?>
					<div class="row">
						<div class="col-xs-12">
							<h2 class="text-center section-title" data-nekoanim="fadeInUp" data-nekodelay="100">
								<span class="section-title--text"><?php echo esc_html( $title ); ?></span>
							</h2>
							<div class="section-descr" data-nekoanim="fadeInUp" data-nekodelay="200">
								<div><?php echo wp_kses_post( $description ); ?></div>
							</div>
						</div>
					</div>
				<?php endif; ?>
				<div class="row">
					<?php if ( $post_meta ) : ?>
						<?php
						$item = 0;
						while ( ! empty( get_field( "preference_3x_{$item}_img" ) ) ) {
							$img_field           = get_field( "preference_3x_{$item}_img" );
							$feature_image       = isset( $img_field['url'] ) ? $img_field['url'] : '';
							$feature_name        = get_field( "preference_3x_{$item}_title" );
							$feature_description = get_field( "preference_3x_{$item}_text" );
							$item++;
							?>
							<div class="col-xs-12 col-sm-6 col-md-4 feature-block">
								<div class="feature-item" data-nekoanim="fadeInDown" data-nekodelay="400">
									<div class="feature-item--icon">
										<img class="b-lazy" src="<?php echo esc_url( get_stylesheet_directory_uri() ); ?>/assets/img/img0.png" data-src="<?php echo esc_url( $feature_image ); ?>" alt="feature_picture">
									</div>
									<h4><?php echo esc_html( $feature_name ); ?></h4>
									<div><?php echo wp_kses_post( $feature_description ); ?></div>
								</div>
							</div>
						<?php } ?>
					<?php endif; ?>
					<?php wp_reset_postdata(); ?>
				</div>
			</div>
		</div>
	</section>
	<?php
}

// Render features block (block_id=60)
function storefront_rs_features_x() {
	$post_meta   = null;
	$title       = '';
	$description = '';

	$query = new WP_Query( array(
		'post_type'  => 'custom_block',
		'meta_query' => array(
			'relation' => 'OR',
			array(
				'key'     => 'block_id',
				'value'   => 60,
				'compare' => '=',
			),
		),
	) );

	while ( $query->have_posts() ) {
		$query->the_post();
		$post_meta = get_post_meta( $query->post->ID );
	}

	if ( $post_meta ) {
		$title       = get_field( 'preference_3x_title' ) ?: '';
		$description = get_field( 'preference_3x_text' ) ?: '';
	}
	?>
	<section class="rs-17">
		<div class="rs-features index-page">
			<div class="container">
				<?php if ( $title || $description ) : ?>
					<div class="row">
						<div class="col-xs-12">
							<h2 class="text-center section-title" data-nekoanim="fadeInUp" data-nekodelay="100">
								<span class="section-title--text"><?php echo esc_html( $title ); ?></span>
							</h2>
							<?php if ( $description ) : ?>
								<div class="section-descr" data-nekoanim="fadeInUp" data-nekodelay="200">
									<div><?php echo wp_kses_post( $description ); ?></div>
								</div>
							<?php endif; ?>
						</div>
					</div>
				<?php endif; ?>
				<div class="row">
					<?php if ( $post_meta ) : ?>
						<?php
						$item = 0;
						while ( ! empty( get_field( "preference_3x_{$item}_img" ) ) ) {
							$img_field           = get_field( "preference_3x_{$item}_img" );
							$feature_image       = isset( $img_field['url'] ) ? $img_field['url'] : '';
							$feature_name        = get_field( "preference_3x_{$item}_title" );
							$feature_description = get_field( "preference_3x_{$item}_text" );
							$item++;
							?>
							<div class="col-xs-12 col-sm-6 col-md-4 feature-block">
								<div class="feature-item" data-nekoanim="fadeInDown" data-nekodelay="400">
									<div class="feature-item--icon">
										<img class="b-lazy" src="<?php echo esc_url( get_stylesheet_directory_uri() ); ?>/assets/img/img0.png" data-src="<?php echo esc_url( $feature_image ); ?>" alt="feature_picture">
									</div>
									<h4><?php echo esc_html( $feature_name ); ?></h4>
									<div><?php echo wp_kses_post( $feature_description ); ?></div>
								</div>
							</div>
						<?php } ?>
					<?php endif; ?>
					<?php wp_reset_postdata(); ?>
				</div>
			</div>
		</div>
	</section>
	<?php
}
