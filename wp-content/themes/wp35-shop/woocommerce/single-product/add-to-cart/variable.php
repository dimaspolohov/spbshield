					...
					<tr>
						<td class="value">
							...
							if ( ! empty( $options ) ) {
								...
								if($name!='pa_color' &&  $product->is_purchasable() && $product->is_in_stock()):
                                   //  var_dump($name);
								?>
									...
									<div class="rs-product__color-list ">
										...
										if ( taxonomy_exists( $name ) ) {
											...
											foreach ( $terms as $term ){
                                               ...
                                               // var_dump($variation->get_stock_quantity());
												if($variation)
                                                $stock_quantity = $variation->get_stock_quantity();

                                                if($stock_quantity)
                                                print_attribute_radio1( $checked_value, $term->slug, $term->name, $sanitized_name );
											}
										} else {
											foreach ( $options as $option ) {
												print_attribute_radio1( $checked_value, $option, $option, $sanitized_name );
											}
										}
										?>
									</div>
