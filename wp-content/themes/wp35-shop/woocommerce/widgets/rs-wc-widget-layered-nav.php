...
add_action( 'widgets_init', 'rs_layered_nav_widget' );

class RS_WC_Widget_Layered_Nav extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_id          = 'rs_woocommerce_layered_nav';
		$this->widget_name        = 'RS Фильтр товаров по атрибутам';
		parent::__construct();
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( 'dropdown' === $display_type ) {
			...
			$found = $this->layered_nav_dropdown( $terms, $taxonomy, $query_type, $widget_title, $is_count );
		} else {
			$found = $this->layered_nav_list( $terms, $taxonomy, $query_type, $widget_title, $is_count );
		}
	...
	protected function layered_nav_list( $terms, $taxonomy, $query_type, $widget_title, $is_count ) {
		...
		//    var_dump($term);
			...
			if ( $count > 0 || $option_is_set ) {
				...
				$term_html = '<a rel="nofollow" href="' . $link . '"><span class="checkbox"></span>' . esc_html( $term->name ) . '</a>';
			} else {
				$link      = false;
