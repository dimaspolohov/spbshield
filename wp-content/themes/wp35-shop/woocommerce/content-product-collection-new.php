...
?>
<div class="rs-collection-catalog__slide swiper-slide">
	<div class="product">
		<a href="<?php echo get_permalink() ?>">
			...
			<div class="product__description">
				...
				if(isset($pa_color)||$prodsVariationColor) {
					ob_start();
					$k = 0;
					if(isset($pa_color)) if($pa_color && isset($pa_color['options']) && count($pa_color['options'])>0) {
						foreach($pa_color['options'] as $color) { $k++;
							?><li style="background-color:#<?=get_term($color)->slug?>"></li><?
						}
					}
					if($prodsVariationColor) foreach($prodsVariationColor as $key => $item) { $k++;
						?><li style="background-color:#<?=$item['color']->slug?>"></li><?
					}
				...
				if(!$product->is_in_stock()){
					?><div class="not_in_stock" style="color:red;margin-top:10px;"><?_e('Нет в наличии')?></div><?
				} else {
					if($onsale||get_field('_new_product')=='yes'||wp_strip_all_tags($price)!='') :?>
					<div class="product__footer">
						...
						<?php if($onsale||get_field('_new_product')=='yes') :?>
